...
@Component({
  ...
  <div style="max-width: 720px; margin: 40px auto; padding: 16px;">
    ...
    <div style="opacity:.75; margin-bottom:14px;">Angular + .NET API template</div>

    <div style="display:flex; gap:8px; margin: 12px 0;">
      <input [formControl]="titleCtrl" placeholder="New todo..."
        style="flex:1; padding:10px; border-radius:12px; border:1px solid #e5e7eb; outline:none;" />
      <button (click)="add()" [disabled]="titleCtrl.invalid || loading()"
        style="padding:10px 14px; border-radius:12px; cursor:pointer;">
        Add
      </button>
    </div>

    <div *ngIf="error()" style="color:#b00020; margin: 10px 0;">{{ error() }}</div>
    ...
    <ul style="list-style:none; padding:0; margin:0;" *ngIf="!loading()">
      <li *ngFor="let t of todos()"
        ...
        <button (click)="remove(t)" [disabled]="loading()"
...
export class TodoListComponent {
  ...
  toggle(t: TodoItem) {
    ...
    this.api.update(t.id, { title: t.title, isDone: !t.isDone }).subscribe({
      next: () => this.todos.set(this.todos().map(x => x.id === t.id ? { ...x, isDone: !x.isDone } : x)),
